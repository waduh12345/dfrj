const translations = {
  "hero-title-1": "Warnai Dunia Anak",
  "hero-title-2": "dengan",
  "hero-title-3": "COLORE",
  "hero-subtitle":
    "Produk seni dan kerajinan ramah lingkungan yang mengembangkan kreativitas anak sambil menjaga kelestarian bumi untuk masa depan mereka.",
  "hero-cta": "Belanja Sekarang",
  "hero-other-1": "Keluarga Puas",
  "hero-other-2": "Produk Kreatif",
  "sec-2-title-1": "Jelajahi Merk",
  "sec-2-title-2": "Produk",
  "sec-2-subtitle":
    "Temukan beragam produk seni dan kerajinan yang dirancang khusus untuk mengembangkan kreativitas anak dengan cara yang menyenangkan dan ramah lingkungan.",
  "sec-2-card-active": "Aktif",
  "sec-2-card-inactive": "Nonaktif",
  "sec-2-card-cta": "Lihat Produk",
  "sec-3-title-1": "Mengapa Pilih",
  "sec-3-title-2": "COLORE?",
  "sec-3-subtitle":
    "Kami berkomitmen memberikan yang terbaik untuk anak-anak dengan produk berkualitas tinggi yang aman dan ramah lingkungan.",
  "sec-3-item-1-title": "100% Ramah Lingkungan",
  "sec-3-item-2-title": "Sertifikat Aman",
  "sec-3-item-3-title": "Mengembangkan Kreativitas",
  "sec-3-item-4-title": "Aktivitas Bersama",
  "sec-3-item-1-content":
    "Semua produk dibuat dari bahan daur ulang dan non-toxic",
  "sec-3-item-2-content":
    "Tersertifikasi aman untuk anak-anak dari berbagai lembaga",
  "sec-3-item-3-content":
    "Dirancang khusus untuk mengasah imajinasi dan keterampilan anak",
  "sec-3-item-4-content":
    "Perfect untuk bonding time keluarga dan aktivitas kelompok",
  "sec-4-title-1": "Produk",
  "sec-4-title-2": "Terlaris",
  "sec-4-subtitle":
    "Produk pilihan yang paling disukai oleh anak-anak dan orang tua di seluruh Indonesia.",
  "sec-4-cta": "Lihat Semua Produk",
  "sec-4-card-reviews": "Ulasan",
  "sec-4-card-cta": "Tambah ke Keranjang",
  "sec-5-title-1": "Yuk Mulai Perjalanan",
  "sec-5-title-2": "Kreatif",
  "sec-5-title-3": "Si",
  "sec-5-title-4": "Kecil",
  "sec-5-title-5": "Hari Ini!",
  "sec-5-subtitle":
    "Bergabunglah bersama ribuan keluarga yang telah mempercayai COLORE untuk menumbuhkan kreativitas anak dengan cara yang aman, menyenangkan, dan ramah lingkungan",
  "sec-5-cta-1": "Mulai Belanja",
  "sec-5-cta-2": "Pelajari Lebih Lanjut",
  "sec-5-item-1-title": "Ramah Lingkungan",
  "sec-5-item-2-title": "Aman untuk Anak",
  "sec-5-item-3-title": "Mengembangkan Kreativitas",
  "sec-5-item-1-content": "100% bahan daur ulang",
  "sec-5-item-2-content": "Tersertifikasi internasional",
  "sec-5-item-3-content": "Dirancang oleh ahli pendidikan",
};

export default translations;
